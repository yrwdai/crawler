...
const weiboday = {
    init() {
        ...
        for (const item of userArr) {
            let _options = selectProxy(`${_config_url.userInfo}${item}`)
    ...
    getUserWeiboInfo:{
        init(pdata,page){
            ...
            let userId = pdata.userInfo.id,
                ...
                _t = this
            let url = `${_config_url.weiboInfo}${userId}&containerid=${containerId}&page=${page}`
            ...
            for(let item of ret){
                if(item.mblog != undefined){
                    let mblogId = item.mblog.id,
                        ...
                        createdAtStr = '',
                        time2 = ''
                    if(createdAt.length <=5 && createdAt.indexOf("昨天")<0){
                        createdAtStr = `${getMonthTime('year')}-${createdAt} 00:00:00:000`
                        time2 = Date.parse(createdAtStr)
                    }
                    if(time2>time || createdAt.indexOf("昨天")>=0){
                        cardsdata = {
                            'blogId': mblogId,
    ...
    getComment:{
        init(userId,mblogId,page){
            let url = `${_config_url.getComment}${mblogId}&page=${page}`,
        ...
        handle(userId,mblogId,commentData,page){
            ...
            for(item of commentData){
                if(item.created_at.indexOf("分钟前")>=0){
                    let _data = {
                        'commentId': `${item.id}`,
